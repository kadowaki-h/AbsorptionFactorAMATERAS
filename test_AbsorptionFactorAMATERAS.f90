! test_AbsorptionFactorAMATERAS.f90
!   H Kadowaki
!
...
! ifort -warn all -O -o test_AbsorptionFactorAMATERAS.ex mod_AbsorptionFactorIN5.f90 test_AbsorptionFactorAMATERAS.f90
!
! OpenMP
! gfortran -fopenmp -Wall -O -o test_AbsorptionFactorAMATERAS.ex mod_AbsorptionFactorIN5.f90 test_AbsorptionFactorAMATERAS.f90
! single CPU, no OpenMP
! gfortran -Wall -O -o test_AbsorptionFactorAMATERAS.ex mod_AbsorptionFactorIN5.f90 test_AbsorptionFactorAMATERAS.f90
!
!-----
program main
!
...
!
  ...
  real(DP) :: psi_amateras , psi , Offset  !!!!!   Omega, 
...
!
 read(*,*) Offset
 write(*,'(/,A,G15.7)')'  Offset = psi - psi_amateras = ',Offset
 
 do j=1,90000
!
! see vectors.pdf
  read(*,*,end=990) psi_amateras, spe_file_name_in, spe_file_name_out
    if(psi_amateras < -3600.0d0) exit
    if(spe_file_name_in(1:3) == 'end' .or. spe_file_name_out(1:3) == 'end' ) exit
  psi = psi_amateras + Offset
  Delta_Omega = - psi
  write(*,'(/,A,3G15.7)')'  psi_amateras, Delta_Omega, psi=', psi_amateras, Delta_Omega, psi
